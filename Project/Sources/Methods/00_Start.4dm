...
Case of 
	...
	Else 
		...
		$options.title:=""
		$options.title:=$options.title+"insert continuous section breaks inside 4D Write Pro documents?"+"\n"
		$options.title:=$options.title+"(and what is the difference with a regular section break?)"
		
		$options.blog:="blog.4d.com"
		$options.info:="4D Write Pro feature"  //ex : "4D View Pro feature"
		
		$options.minimumVersion:="1930"  // 1930 means 19R3   1701 means 17.1 (do not use !)
